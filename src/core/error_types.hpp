...
enum class ErrorMode : std::uint8_t {
	...
	CRASH_ON_ERROR = 5,		// -||- at error
	CRASH_ON_WARNING = 6	// -||- at warning
};
...
inline const char* getErrorMessage(ErrorCode code) {
    switch (code) {
        case ErrorCode::PIXEL_OUT_OF_BOUND_X:	return "DISPLAY: PIXEL_OUT_OF_BOUND_X";
        case ErrorCode::PIXEL_OUT_OF_BOUND_Y:   return "DISPLAY: PIXEL_OUT_OF_BOUND_Y";
...
inline const char* modeToString (ErrorMode mode) {
	switch (mode) {
		...
		case ErrorMode::CRASH_ON_WARNING:	return "CRASH_ON_WARNING";
		default:							return "UNKN";
	}
}
